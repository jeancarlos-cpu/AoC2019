...
const formatData = wire => {
  const data = wire.split(",");
  const path = new Set();
  let x = 0,
    y = 0;
  for (let i = 0; i < data.length; i++) {
    const distance = Number(data[i].slice(1));
    const direction = data[i][0];
    const xPath = direction === "L" ? -1 : direction === "R" ? 1 : 0;
    const yPath = direction === "U" ? -1 : direction === "D" ? 1 : 0;
...
const findIntersection = (path1, path2) => [...path1].filter(x => path2.has(x));

const shortestDistance = intersections =>
  intersections
    .map(e => e.split(",").map(Number))
    .map(e => Math.abs(e[0]) + Math.abs(e[1]))
    .reduce((min, e) => (e < min ? e : min), Infinity);

const path1 = formatData(wire1);
const path2 = formatData(wire2);
const intersections = findIntersection(path1, path2);
console.log(shortestDistance(intersections));
